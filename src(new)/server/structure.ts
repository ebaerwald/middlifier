import { ServerInfo, ExpressOptions, Secrets, Structure, MidFuncs, ReqConfig, Responses, Methods, Routes, InnerFunc } from "..";
import { _encode, _create, _arrayToString } from "../helper";
...
export function buildServerStructure(express?: ExpressOptions, secrets?: Secrets, port?: number, structure?: Structure): ServerInfo
{
    buildEntryFile(express, secrets, port, structure);
    return serverInfo;
}

function buildEntryFile(express?: ExpressOptions, secrets?: Secrets, port?: number, structure?: Structure): ServerInfo
{
    let configPointer = ["server", "structure", "funcs"];
    const { routes, funcs } = structure ?? {};
    const f = funcs;
    const { cors, json, urlencoded, morgan, sets, cookies } = express ?? {};
    let lines = [`import express from "express";`];
    if (cors) lines.push(`import cors from "cors";`);
    if (secrets) lines.push(`import dotenv from "dotenv";`);
    if (morgan) lines.push(`import morgan from "morgan";`);
    if (cookies) lines.push(`import cookieParser from "cookie-parser";`);
    let imports: { [path: string]: string[] } = {};
    if (routes)
    {
        for (const r of routes)
        {
            const { path, name, routes, route } = r;
            route ? `${route}, `: '';
            if (!imports[path]) imports[path] = [];
            imports[path].push(name);
            lines.push(`app.use("${route}", ${name});`);
    ...
    if (f)
    {
        ...
        {
            configPointer.push(i.toString());
            const { path, funcs } = f[i];
            const result = buildFuncs(funcs, `${path}.ts`, configPointer);
            lines.push(...result);
            // for (const f of funcs)
            // {
            //     if (!f.routeKey) lines.push(`app.${f.method?.toLowerCase() ?? 'use'}("${f.route}", ${f.name});`);
            // }
        }
    }
    lines = [
        ...Object.entries(imports).map(([path, names]) => `import { ${names.join(', ')} } from "${path}";`),
        ...lines,
        '',
        `const app = express();`,
        ''
    ]
    if (cors) lines.push(`app.use(cors(${_encode(cors)}));`);
    if (json) lines.push(`app.use(express.json(${_encode(json)};`);
    if (urlencoded) lines.push(`app.use(express.urlencoded(${_encode(urlencoded)}));`);
    if (morgan) lines.push(`app.use(morgan(${morgan}));`);
    if (cookies) lines.push(`app.use(cookieParser());`);
    for (const setKey in sets)
    {
        const set = sets[setKey];
        lines.push(`app.set('${setKey}', ${_encode(set)})`);
    }
    _create('index.ts', {content: _arrayToString([
        ...lines,
        '',
        'app.listen(port, () => {',
...
function buildFuncs(midFuncs: InnerFunc[], pathFromEntryPoint: string = '', configPointer: string[]): string[]
{
    ...
    for (let i = 0; i < midFuncs.length; i++)
    {
        ...
        {
            const currentPath = getPathFromEntryPoint(pathFromEntryPoint, path);
            buildFunc(`${currentPath}.ts`, name, req ?? {}, res ?? {}, getSubFuncs(funcs ?? []), [...configPointer, i.toString()]);
            if (funcs) return buildFuncs(funcs, currentPath, [...configPointer, i.toString()]);
        }
    }
}
...
function buildFunc(path: string, name: string, req: ReqConfig, res: ResConfig, subFuncs: [string[], FuncsInfo], configPointer: string[])
{
